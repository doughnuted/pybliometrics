...
class ArticleMetadata(Search):
    """Class to retrieve article metadata from ScienceDirect."""

    @property
    def results(self) -> list[namedtuple] | None:
        """
        A list of namedtuples in the form `(authorKeywords authors available_online_date
        first_author abstract_text doi title eid link openArchiveArticle
        openaccess_status openaccessArticle openaccessUserLicense pii aggregationType
        copyright coverDate coverDisplayDate edition endingPage isbn publicationName
        startingPage teaser api_link publicationType vor_available_online_date)`.
        ...
        for item in self._json:
            ...
            authors_list = [
                author.get("$")
                for author in chained_get(item, ["authors", "author"], [])
            ]
            ...
            doi = (
                item.get("prism:doi") or item.get("dc:identifier")[4:]
                if item.get("dc:identifier")
                else None
            )
            new = doc(
