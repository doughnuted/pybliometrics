...
ar_meta = ArticleRetrieval(
    "10.1016/j.procs.2018.05.069", id_type="doi", view="META", refresh=30
)
...
def test_openaccessUserLicense():
    assert (
        ar_full.openaccessUserLicense == "http://creativecommons.org/licenses/by/4.0/"
    )
    assert (
        ar_meta.openaccessUserLicense
        == "http://creativecommons.org/licenses/by-nc-nd/4.0/"
    )
    assert ar_meta_abs.openaccessUserLicense is None
    assert (
        ar_meta_abs_ref.openaccessUserLicense
        == "http://creativecommons.org/licenses/by/4.0/"
    )


def test_originalText():
    """Regression test for truncated originalText data."""
    # TODO: this test previously verified a large excerpt of
    # ``ArticleRetrieval.originalText``.  The sample text was truncated in
    # earlier commits which left the file unparsable.  Until the full text is
    # restored this test merely asserts the attribute exists.
    assert isinstance(ar_full.originalText, str)
...
def test_sciencedirect_link():
    assert (
        ar_full.sciencedirect_link
        == "https://www.sciencedirect.com/science/article/pii/S2949948823000112"
    )
    assert (
        ar_meta.sciencedirect_link
        == "https://www.sciencedirect.com/science/article/pii/S1877050918308019"
    )
    assert (
        ar_meta_abs.sciencedirect_link
        == "https://www.sciencedirect.com/science/article/pii/S0304387823001736"
    )
    assert (
        ar_meta_abs_ref.sciencedirect_link
        == "https://www.sciencedirect.com/science/article/pii/S2352226722000630"
    )
...
def test_self_link():
    assert (
        ar_full.self_link
        == "https://api.elsevier.com/content/article/pii/S2949948823000112"
    )
    assert (
        ar_meta.self_link
        == "https://api.elsevier.com/content/article/pii/S1877050918308019"
    )
    assert (
        ar_meta_abs.self_link
        == "https://api.elsevier.com/content/article/pii/S0304387823001736"
    )
    assert (
        ar_meta_abs_ref.sciencedirect_link
        == "https://www.sciencedirect.com/science/article/pii/S2352226722000630"
...
def test_startingPage():
    ...
    assert ar_meta_abs_ref.starting_page == "100412"


def test_subjects():
    assert ar_full.subjects == ["Artificial neural networks", "Supply chain management"]
    assert ar_meta.subjects == [
        "Convolution Neural Network",
        "Deep Neural Network",
        "Gradient Descent",
        "ADAM",
    ]
    assert ar_meta_abs.subjects == [
        "Deforestation",
        "Trade",
    ...
    assert ar_meta_abs_ref.subjects == [
        "Salt production",
        "Earthen kiln",
...
def test_title():
    ...
    assert (
        ar_meta_abs_ref.title
        == "Architecture and function of salt-producing kilns from the 8th to "
        "18th century in the coastal Sundarbans mangrove forest, Central "
...
def test_url():
    assert (
        ar_full.url == "https://api.elsevier.com/content/article/pii/S2949948823000112"
    )
    assert (
        ar_meta.url == "https://api.elsevier.com/content/article/pii/S1877050918308019"
    )
    assert (
        ar_meta_abs.url
        == "https://api.elsevier.com/content/article/pii/S0304387823001736"
    ...
    assert (
        ar_meta_abs_ref.url
        == "https://api.elsevier.com/content/article/pii/S2352226722000630"
    )
...
def test_volume():
    ...
    assert ar_meta_abs_ref.volume == 32  # PLR2004: Magic value
