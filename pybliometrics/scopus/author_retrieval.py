"""Retrieve author-level metadata from Scopus."""
...
class AuthorRetrieval(Retrieval):
    @property
    def affiliation_current(self) -> list[namedtuple] | None:
        """
    ...
    @property
    def affiliation_history(self) -> list[namedtuple] | None:
        """
    ...
    def citation_count(self) -> int:
        ...
        return make_int_if_possible(
            chained_get(self._json, ["coredata", "citation-count"])
        )
    ...
    def cited_by_count(self) -> int:
        ...
        return make_int_if_possible(
            chained_get(self._json, ["coredata", "cited-by-count"])
        )
    ...
    def classificationgroup(self) -> list[tuple[int, int]] | None:
        ...
        out = [
            (int(filter_digits(item["$"])), int(filter_digits(item["@frequency"])))
    ...
    def given_name(self) -> str | None:
        ...
        return html_unescape(
            chained_get(self._profile, ["preferred-name", "given-name"])
        )

    @property
    def h_index(self) -> str | None:
    ...
    def identifier(self) -> int:
        ...
        if ident != self._id:
            text = (
                f"Profile with ID {self._id} has been merged and the new "
    ...
    def indexed_name(self) -> str | None:
        ...
        if self._view in ("STANDARD", "ENHANCED"):
            indexed_name = html_unescape(
                chained_get(self._profile, ["preferred-name", "indexed-name"])
            )
        elif self._view == "LIGHT":
            ...
            if name_variants:
                indexed_name = chained_get(
                    name_variants[0], ["name-variant", "indexed-name"]
                )
            else:
                ...
                indexed_name = " ".join(
                    [
                        preferred_name.get("initials", ""),
                        preferred_name.get("surname", ""),
                    ]
                )
        else:
            indexed_name = None
    ...
    def name_variants(self) -> list[namedtuple] | None:
        ...
        out = [
            variant(
                indexed_name=html_unescape(var["indexed-name"]),
                surname=html_unescape(var["surname"]),
    ...
    def subject_areas(self) -> list[namedtuple] | None:
        ...
        areas = [
            area(area=item["$"], code=int(item["@code"]), abbreviation=item["@abbrev"])
        ...
        **kwds: str,
        ...
        if self._view in ("METRICS", "LIGHT", "STANDARD", "ENHANCED"):
            ...
            try:
                ...
                text = (
                    f"Author profile with ID {author_id} has been merged and "
    ...
    def __str__(self):
        ...
        if self._view in ("STANDARD", "ENHANCED", "LIGHT"):
            ...
            s = (
                f"{self.indexed_name} from {main_aff.preferred_name} in "
    ...
    def get_coauthors(self) -> list[namedtuple] | None:
        """
    ...
    def get_documents(
        self, subtypes: list[str] | None = None, *args: str, **kwds: str
    ...
    def get_document_eids(self, *args: str, **kwds: str) -> list[str] | None:
        """
    ...
    def estimate_uniqueness(
        self, query: str | None = None, *args: str, **kwds: str
    ) -> int:
        """
