...
CONFIG_FILE = next(
    (path for path in config_path_options if path.exists()), config_path_options[-1]
)
...
CACHE_PATH = next(
    (path for path in cache_path_options if path.exists()), cache_path_options[-1]
)
...
DEFAULT_PATHS = {
    ...
    "ScDirSubjectClassifications": CACHE_PATH
    / "ScienceDirect"
